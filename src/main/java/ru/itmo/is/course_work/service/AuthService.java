...
public class AuthService {
  ...
  public JwtResponse refresh(@NotNull String refreshToken) {
    if (jwtProvider.validateRefreshToken(refreshToken)) {
      ...
      if (saveRefreshToken != null && saveRefreshToken.contains(refreshToken)) {
        ...
        Optional<RefreshStorage> oldToken =
            refreshStorageRepository.findByRefreshToken(refreshToken);
        if (oldToken.isEmpty()) {
          log.error(
              "Ошибка обновления существующего refresh токена в бд. Refresh токен: {}",
              refreshToken);
          throw new CustomException(ExceptionEnum.SERVER_ERROR);
        }
  ...
  public void signOutFromAllDevices(String refreshToken) {
    if (jwtProvider.validateRefreshToken(refreshToken)) {
      refreshStorageRepository.deleteByUserId(
          Optional.ofNullable(RoleService.getCurrentUser())
              .orElseThrow(
                  () -> {
