...
public class JwtProvider {
    ...
    @Value("${jwt.access.life-time:60}")
    private Integer accessTokenLifetime;

    @Value("${jwt.refresh.life-time:1440}")
    private Integer refreshTokenLifetime;

    public JwtProvider() {
        log.warn("JWT provider initialized in stub mode. Tokens are fake.");
    }

    public String generateAccessToken(@NonNull User user) {
        return "fake-access-token-for-user-" + user.getId();
    }
    ...
    public Claims getAccessClaims(@NonNull String token) {
        ...
        // извлекаем id из токена, если он сгенерирован stub-методом
        if (token.startsWith("fake-access-token-for-user-")) {
            String userId = token.replace("fake-access-token-for-user-", "");
            claims.put("userId", userId);
        } else {
            // fallback
            claims.put("userId", "stub-user");
        }
        return claims;
    }
    ...
    public Claims getRefreshClaims(@NonNull String token) {
        Claims claims = Jwts.claims();
        if (token.startsWith("fake-refresh-token-for-user-")) {
            String userId = token.replace("fake-refresh-token-for-user-", "");
            claims.put("userId", userId);
        } else {
            claims.put("userId", "stub-user");
        }
...
//
//    PKCS8EncodedKeySpec keySpecAPr =
//        new PKCS8EncodedKeySpec(Base64.getDecoder().decode(accessPr.getBytes()));
//    PrivateKey accessPrivateKey = kf.generatePrivate(keySpecAPr);
//    X509EncodedKeySpec keySpecAPub =
//        new X509EncodedKeySpec(Base64.getDecoder().decode(accessPub.getBytes()));
//    RSAPublicKey accessPublicKey = (RSAPublicKey) kf.generatePublic(keySpecAPub);
//
//    PKCS8EncodedKeySpec keySpecRPr =
//        new PKCS8EncodedKeySpec(Base64.getDecoder().decode(refreshPr.getBytes()));
//    PrivateKey refreshPrivateKey = kf.generatePrivate(keySpecRPr);
//    X509EncodedKeySpec keySpecRPub =
//        new X509EncodedKeySpec(Base64.getDecoder().decode(refreshPub.getBytes()));
...
//        Instant.now().plus(accessTokenLifetime, ChronoUnit.MINUTES);
...
//        Instant.now().plus(refreshTokenLifetime, ChronoUnit.MINUTES);
