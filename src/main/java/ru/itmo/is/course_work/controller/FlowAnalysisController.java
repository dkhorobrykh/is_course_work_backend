...
public class FlowAnalysisController {
  ...
  @Operation(summary = "Возвращает полный анализ потока для всех рейсов.")
  @GetMapping
  ...
  @Operation(
      summary =
          "Возвращает сводную информацию о каждом рейсе (его название, количество пассажиров и общий вес груза).")
  @GetMapping("/summary")
    ...
    return analysis.stream()
        .map(
            dto ->
                Map.of(
                    "flight",
                    (Object) dto.getFlight().getName(),
                    "passengers",
                    (Object) dto.getPassengerCount(),
                    "totalCargoWeight",
                    (Object) dto.getTotalCargoWeight()))
