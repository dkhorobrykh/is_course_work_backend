...
public enum ExceptionEnum {
  ...
  PREFLIGHT_CHECK_FAILED_PROTECTION(
      "Проверка перед полетом не пройдена: системы защиты не активированы.",
  ...
  PASSENGER_AND_CARGO_SHOULD_BE_IN_DIFFERENT_INSURANCES(
      "Пассажир и груз должны быть оформлены разными страховками", HttpStatus.BAD_REQUEST),
  ...
  WRONG_INSURANCE_PROGRAM_IS_NOT_ACTIVE_AT_FLIGHT_DATE(
      "Программа страхования недоступна в момент перелета", HttpStatus.BAD_REQUEST),
  ...
  ISSUE_DATE_MUST_BE_BEFORE_EXPIRATION_DATE(
      "Дата выдачи должна быть раньше даты истечения срока действия документа",
      HttpStatus.BAD_REQUEST),
  ...
  FLIGHT_SHOULD_BE_LANDED_BEFORE_UNLOADING_CARGO(
      "Рейс должен приземлиться перед выгрузкой груза", HttpStatus.BAD_REQUEST),
  ...
  FLIGHT_SHOULD_BE_APPROVED_BEFORE(
      "Перед взаимодействием с грузом рейс должен быть подтвержден", HttpStatus.BAD_REQUEST),
