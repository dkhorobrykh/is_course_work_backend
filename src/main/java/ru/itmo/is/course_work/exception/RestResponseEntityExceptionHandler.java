...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  @Order(1)
  protected ResponseEntity<ErrorDetails> handleCustomException(
      CustomException ex, WebRequest request) {
    ExceptionEnum ExceptionEnum = ex.getExceptionEnum();

    return new ResponseEntity<>(
        new ErrorDetails(
            Clock.systemDefaultZone().instant(), ExceptionEnum, request.getDescription(false)),
        ExceptionEnum.getHttpStatus());
  }

  @ExceptionHandler(AccessDeniedException.class)
  @Order(9_000)
  protected ResponseEntity<ErrorDetails> handleAuthorizationException(
